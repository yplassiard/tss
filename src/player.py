...
class Player(Object):
    ...
    def __init__(self, name, config):
        ...
        self.maxHealth = gameconfig.get_value(
            config, "health", int, {"minValue": 1})
        self.maxMagic = gameconfig.get_value(config, "magic", int, {})
        self.maxStamina = gameconfig.get_value(
            config, "stamina", int, {"minValue": 1})
        self.maxDistance = gameconfig.get_value(
            config, "max-distance", float, {"defaultValue": 5})
        self.halth = self.maxHealth
        self.magic = 0
        self.staminaRecoveryTime = gameconfig.get_value(
            config, "stamina-recovery-time", int, {"defaultValue": constants.CHARACTER_STAMINA_RECOVERY_TIME})
        self.staminaDecrement = gameconfig.get_value(
            config, "stamina-decrement", int, {"defaultValue": 1})
        self.staminaIncrement = gameconfig.get_value(
            config, "stamina-increment", int, {"defaultValue": 1})
    ...
    def loadInventory(self, inventory):
        ...
        for name in inventory:
            ...
            if obj is not None:
                ...
                logger.error(
                    self, "Cannot add {name} to inventory: object not found.".format(name=name))
                return False
        return True
    ...
    def event_character_spawn(self, evt):
        ...
        if s is not None:
            ...
            scene = 'Unknown'
        logger.info(self, "Spawned in {scene}, at position {pos}".format(
            scene=scene, pos=evt.get("position", 'Unknown')))
    ...
    def event_scene_interval_tick(self, evt):
        ...
        if delta > self.staminaRecoveryTime:
            ...
            logger.info(self, "Recovering stamina {s}%, delta+{delta}".format(
                s=(self.stamina / self.maxStamina * 100), delta=delta))
