...
class SceneManager():
    ...
    def create_scene(self, config):
        ...
        if cls is None:
            raise RuntimeError(
                "Scene type {type} is not known.".format(type=scene_type))
    ...
    def stack_scene(self, name):
        ...
        if my_scene.name == self._active_scene.name \
           or (len(self._stack) >= 1 and self._stack[-1].name == my_scene.name):
            logger.error(
                self, f"Cannot stack {my_scene.name} on top of itself")
            return False
    ...
    def event_scene_interval_activate(self, event):
        ...
        if my_scene is None:
            raise RuntimeError(
                "Invalid call to event_scene_interval_activate without a target scene.")
    ...
    def event_scene_interval_deactivate(self, event):
        ...
        for interval_scene in self._intervalScenes:
            ...
            idx += 1
        logger.error(
            self, f"Failed to remove scene from interval scenes: {my_scene.name}")
        ...
        for interval_scene in self._intervalScenes:
            if interval_scene.get_next_tick() <= now:
                try:
                    ...
                    logger.exception(self, "Failed to execute {cls}.event_interval: {exception}".format(
                        cls=interval_scene.__class__.__name__, exception=ex), ex)
    ...
    def execute(self, script, data=None, target=None):
        ...
        for obj in obj_list:
            ...
            if method is not None:
                try:
                    logger.info(self, "Executing {name}.{script}".format(
                        name=cls.__name__, script=script))
                    if script.startswith('input'):
                        method()
                        return True
                    method(data)
                except Exception as ex:
                    logger.error(self, "Failed to execute {name}.{script}: {exception}".format(
                        name=cls.__name__, script=script, exception=ex))
                    logger.exception(self, "{name}.{script}".format(
                        name=cls.__name__, script=script), ex)
                    return False
        return False
...
def initialize():
    ...
    if _INSTANCE is None:
        try:
            ...
            logger.exception(
                "scene_manager", "Failed to initialize scene manager", ex)
            return False
    ...
    try:
        my_dir = os.scandir(os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "scenes"))
    except Exception as ex:
        logger.error(
            _INSTANCE, "Failed to load secenes: {exception}".format(exception=ex))
        return False
    for entry in my_dir:
        match = re.match(r"(^[^#]+.*)\.py$", entry.name)
        if match is not None and entry.name != 'scene.py' and entry.name != '__init__.py':
            logger.info(_INSTANCE, "Loading scene {name}".format(
                name=match.group(1)))
            total_scenes += 1
            try:
                ...
                logger.exception(
                    _INSTANCE, f"Failed to instanciate scene {match.group(1)}", ex)

    try:
        my_dir = os.scandir(os.path.join(
            os.path.abspath("."), "data", "scenes"))
    except Exception as ex:
    ...
    if my_dir is not None:
        for entry in my_dir:
            ...
            if match is not None:
                try:
                    json_config_list = gameconfig.load_scene_configuration(
                        entry.name)
                except Exception as ex:
                    json_config_list = None
                if json_config_list is None:
                    logger.error(
                        "scene_manager", "Failed to load scene {name}".format(name=entry.name))
                    continue
                for json_config in json_config_list:
                    ...
                    if json_config is not None:
                        try:
                            ...
                            logger.error(_INSTANCE, "Failed to create scene {file}: {exception}".format(
                                file=entry.name, exception=ex))
                            continue
                        if obj is not None:
    ...
    if total_scenes > loaded_scenes:
        logger.error(_INSTANCE, "{count} scenes failed to load".format(
            count=total_scenes - loaded_scenes))
