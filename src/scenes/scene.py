        ...
        if config:
            ...
            if self._interval is None or isinstance(self._interval, int) is False or self._interval < constants.SCENE_MININUM_INTERVAL:
                msg = "Invalid interval value {value}; has to be integer, minimum is {minValue}".format(
                    value=self._interval, minValue=constants.SCENE_MININUM_INTERVAL)
...
class MapRegionScene(IntervalScene):
    ...
    def __init__(self, name, config):
        ...
        self.height = gameconfig.get_value(
            config, "height", int, {"minValue": 1})
        self.width = gameconfig.get_value(
            config, "width", int, {"minValue": 1})
        self.regionLinks = gameconfig.get_value(
            config, "region-links", list, {"elements": 1})
        self.objectConfigs = gameconfig.get_value(
            config, "objects", list, {"defaultValue": []})
        self.walkSounds = gameconfig.get_value(
            config, "walking", list, {"elements": 1, "defaultValue": []})
        cameraModeStr = gameconfig.get_value(
            config, "default-camera-mode", str, {"defaultValue": "top"})
        if cameraModeStr not in ["top", "subjective"]:
            raise RuntimeError(
                "The camera mode has to be set either to 'top' or 'subjective'.")
        if cameraModeStr == "top":
            ...
            raise RuntimeError(
                "Unknown camera mode {mode}".format(mode=cameraModeStr))
    ...
    def loadObjects(self):
        ...
        for objConfig in self.objectConfigs:
            ...
            if objPos is None or isinstance(objPos, list) is False or len(objPos) != 2:
                logger.error(
                    self, "Object({name}) has no position".format(name=obj.name))
                continue
            if objPos[0] <= 0 or objPos[0] >= self.width or objPos[1] <= 0 or objPos[1] >= self.height:
                logger.error(self, "Object({name}, {pos}) position is outside this scene.".format(
                    name=obj.name, pos=objPos))
                continue
            logger.info(self, "{cls}({name}) added to the scene".format(
                cls=obj.__class__.__name__, name=obj.name))
            ...
            if enter is not None:
                for region in self.regionLinks:
                    if region.get("name", None) == enter:
                        pos = gameconfig.get_value(
                            region, "position", list, {"elements": 4})
                        msg = "Entering {name} {enter}: {pos}".format(
                            name=self.name, enter=enter, pos=pos)
                        logger.debug(self, msg)
                        speech.speak(msg)
                        if pos[0] == pos[2]:
                            self.playerPosition = [
                                pos[0] + 1 if pos[0] == 0 else self.width - 2, pos[1] + int((pos[3] - pos[1]) / 2)]
                        elif pos[1] == pos[3]:
                            self.playerPosition = [
                                pos[0] + int((pos[2] - pos[0]) / 2), pos[1] + 1 if pos[1] == 0 else self.height - 2]
                        else:
                            self.playerPosition = [
                                int(self.width / 2), int(self.height / 2)]
            else:
                logger.info(
                    self, "No enter specified, spawning in the middle.")
                self.playerPosition = [
                    int(self.width / 2), int(self.height / 2)]
        else:
            logger.info(self, "No params specified, spawning in the middle.")
            self.playerPosition = [int(self.width / 2), int(self.height / 2)]
        event_manager.post(event_manager.CHARACTER_SPAWN, {
                           "scene": self, "position": self.playerPosition})

    def describe(self):
        self.speechDescription = " ({position})".format(
            position=self.playerPosition)
        if self.cameraMode == constants.CAMERA_SUBJECTIVE:
            self.speechDescription += ", direction {dir} degrés".format(
                dir=self.curAngle / constants.DEF_PI_DIVIDE * 180)
        super().describe()
    ...
    def input_press_tab(self):
        if self.cameraMode == constants.CAMERA_TOP:
            ...
            self.curAngle = 80
            event_manager.post(event_manager.AUDIO_CAMERA_CHANGE, {
    ...
    def onWalk(self, running=False, direction=None):
        ...
        event_manager.post(event_manager.CHARACTER_MOVE, {
                           "direction": self.direction, "type": "run" if running is True else "walk"})
    ...
    def event_will_character_move(self, evt):
        ...
        objs = object_manager.getNearestObjects(
            newPos, self.player, self.objects)
    ...
    def event_character_move(self, evt):
        ...
        import object_manager
        objs = object_manager.getNearestObjects(
            self.playerPosition, self.player, self.objects)
        if objs is not None and len(objs) > 0:
            distance, obj = objs[0]  # takes the nearest object
            logger.info(self, "{name} at {distance}".format(
                name=obj, distance=distance))
            if distance <= obj.getInteractionDistance():
                if obj.onInteract(self, self.player) is True:
                    self.stopMoving()

        footStepSound = self.getGroundTypeSound()
        audio.play(footStepSound[0], footStepSound[1],
                   audio.computePan(0, self.width, newPos[0]))
        ...
        if core.get_current_ticks() - self.objectEchoTick > constants.OBJECT_ECHO_TIME:
            ...
            for distance, obj in objs:
                ...
                if diffX <= -0.8:
                    ...
                    diffX = 0.8
                volume = (constants.AUDIO_FX_VOLUME - (distance / self.player.getMaxDistance())
                          if distance <= self.player.getMaxDistance() else 0)
                if volume <= 0:
                    return
                ...
                if self.cameraMode == constants.CAMERA_TOP:
                    audio.play(obj.getSignalSound(), volume, diffX,
                               pitch=audio.computePitch(0.7, 1.3, diffY))
            self.objectEchoTick = core.get_current_ticks()
            if self.cameraMode == constants.CAMERA_SUBJECTIVE:
                event_manager.post(
                    event_manager.AUDIO_PLAY_3D, {"scene": self})
