...
class ObjectManager(object):
    ...
    def load(self, config):
        ...
        if name is None:
            logger.error(
                self, "Each object has to be named with a \"name\" property")
            return False
        if self.objects.get(name, None) is not None:
            logger.error(
                self, "The object's name \"{name}\" is already defined".format(name=name))
            return False
        type = gameconfig.get_value(config, "type", str)
        cls = objectsMap.get(type, None)
        if cls is None:
            logger.error(self, "{name}'s type {type} is not known".format(
                name=name, type=type))
            return False
        try:
            o = cls(name, config)
        except Exception as e:
            logger.exception(self, "Error instanciating {cls}({name}): {exception}".format(
                cls=cls.__name__, name=name, exception=e), e)
            return False
        self.objects[name] = o
        return o
    ...
    def get(self, name):
        ...
        if o is None:
            logger.warning(
                self, "Object {name} is not known".format(name=name))
            return o
...
def initialize():
    ...
    if _instance is None:
        try:
            ...
            logger.error(
                "ObjectManager", "Failed to initialize object manager: {e}".format(e=e))
            return False
    return True
