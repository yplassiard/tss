...
class Object:
    ...
    def __init__(self, name, config):
        ...
        self.position = gameconfig.get_value(
            config, "position", list, {"elements": 2})
        self.size = gameconfig.get_value(config, "size", list, {"elements": 2})
        self.signalSound = "signal-sound"
        self.interactDistance = gameconfig.get_value(
            config, "interact-distance", float, {"defaultValue": -1})
        self.minDistance = gameconfig.get_value(
            config, "min-distance", float, {"defaultValue": 0.5})
        self.maxDistance = gameconfig.get_value(
            config, "max-distance", float, {"defaultValue": constants.OBJECT_MAX_DISTANCE})
...
class Chest(Openable):
    ...
    def __init__(self, name, config):
        ...
        self.openSound = config.get(
            "open-sound", gameconfig.getContainerOpenSound())
        self.openSoundVolume = config.get(
            "open-sound-volume", gameconfig.getContainerOpenSoundVolume())
...
class Key(Seizable):
    ...
    def __init__(self, name, config):
        super().__init__(name, config)
        self.target = gameconfig.get_value(
            config, "target", str, {"defaultValue": None})
        if self.target is None:
            raise RuntimeError(
                "Key({name}) without any target to unlock.".format(name=name))
    ...
    def use(self, target):
        ...
        if obj is None:
            logger.error(self, "Target {target} not found".format(
                target=self.target))
            return False
        if issubclass(obj, Openable) is False:
            logger.error(self, "Target {target} is not unlockable".format(
                target=self.target))
        try:
            ret = obj.unlock(self)
        except Exception as e:
            logger.error(self, "Unable to unlock {name} with {this}: {exception}".format(
                name=obj.name, this=self.name, exception=e))
            ret = False
        return ret
...
i.e when using this object."""
    ...
    def __init__(self, name, config):
        super().__init__(name, config)
        self.size = (1, 1)
        scene = gameconfig.get_value(
            config, "scene", str, {"defaultValue": None})
        interact = gameconfig.get_value(
            config, "auto-interact", str, {"defaultValue": 'never'})
        if interact not in ["never", "once", "always"]:
            raise RuntimeError(
                "auto-interact value {v} should be one of 'never', 'once', 'always'".format(v=interact))
        ...
        self.hitMax = gameconfig.get_value(
            config, "hit-count", int, {"defaultValue": -1})
        self.hitCount = 0
        self.hitScene = gameconfig.get_value(
            config, "hit-scene", str, {"defaultValue": None})
    ...
    def event_did_object_hit(self, evt):
        if self.hitMax != -1 and self.hitCount == self.hitMax:
            ...
            if scene_manager.scene_exists(self.hitScene) is False:
                logger.error(self, "{scene} does not exist".format(
                    scene=self.hitScene))
                return False
