...
class AudioManager():
    ...
    def __init__(self, config):
        ...
        try:
            ...
            logger.exception(
                self, "Failed to load FMOD library: {e}".format(e=e), e)
            raise e
        self.fadeInterval = gameconfig.get_value(
            config, "fade-interval", int, {"defaultValue": constants.AUDIO_FADE_INTERVAL, "minValue": 1})
        self.fadeTicks = 0
        self.stereoWidth = gameconfig.get_value(config, "stereo-field-width", int, {
                                                "defaultValue": constants.AUDIO_STEREO_FIELD_WIDTH, "minValue": 30, "maxValue": 100})
        self.stereoWidth /= 100
        for s in gameconfig.get_sound_resources():
            logger.info(self, "Loading {name} ({file})".format(
                name=s["name"], file=s["file"]))
            try:
                snd = sound.Sound(self.fmod, s["name"], s["file"])
                self.soundMap[s["name"]] = snd
            except Exception as e:
                logger.exception(self, "Failed to load {file}: {exception}".format(
                    file=s["file"], exception=e), e)
        global _instance
    ...
    def loadMusic(self, config):
        ...
        if loaded is not None:
            return True
        try:
            logger.info(self, "Loading music {name} ({file}, {volume})".format(
                name=name, file=file, volume=volume))
            snd = sound.Music(self.fmod, name, file, loops=loop, volume=volume)
        except Exception as e:
            logger.error(self, "Cannot load music {name} ({file}): {exception}".format(
                name=name, file=file, exception=e))
            return False
    ...
    def play(self, name, volume, pan=0.0, pitch=1.0):
        ...
        if snd is not None:
            try:
                ...
                logger.exception(self, "Error playing {name}: {exception}".format(
                    name=name, exception=e), e)
                return False
        else:
    ...
    def event_leave_scene(self, event):
        ...
        for music in self.musicMap:
            ...
            if not found:
                ...
                newVolume = gameconfig.get_value(musicConfig, "volume", float, {
                                                 "defaultValue": constants.AUDIO_FX_VOLUME})
                initialVolume = gameconfig.get_value(
                    musicConfig, "initial-volume", float, {"defaultValue": 0.0})
                ...
                if newVolume != snd.getVolume():
                    logger.info(self, "Scheduling {music} to new volume {newVolume}".format(
                        music=music, newVolume=newVolume))
                    effects.timeEffects.append(
                        effects.VolumeEffect(self.musicMap[music], newVolume))
    ...
    def event_audio_render(self, evt):
        ...
        if scene is None or listenerPos is None or dirVector is None:
            logger.error(
                self, "cannot render, missing scene or listener position")
            return False
    ...
    def event_stack_scene(self, evt):
        ...
        for music in sceneMusics:
            ...
            if snd is None:
                logger.error(self, "stack({name}): Music {music} not found.".format(
                    name=scene.name, music=music["name"]))
                continue
            snd.play()
    ...
    def event_scene_interval_tick(self, event):
            ...
            for effect in effects.timeEffects:
                if isinstance(effect, effects.VolumeEffect):
                    ...
                    if effect.isCompleted():
                        logger.info(
                            self, "Effect {e} completed".format(e=effect))
                        effects.timeEffects.remove(effect)
                        continue
        ...
        if self.fmod is not None:
            try:
                ...
                logger.exception(
                    self, "Error updating fMOD: {e}".format(e=e), e)
...
def initialize(config):
    if _instance is None:
        try:
            ...
            logger.exception(
                "audio", "Error initializing audio: {exception}".format(exception=e), e)
        return False
    else:
        return True
