...
class Logger:
    ...
    def log(self, category, system, message):
        ...
        if logLevel < level:
            return
        try:
            self.logIO.write(bytes(f"{datetime.datetime.utcnow().isoformat()} {systemName} {category}: {message}\r\n", 'utf-8'))
            # self.logIO.flush()
        except Exception as e:
            print("Log failed: {system} {category}: {message}. Caused by {exception}".format(
                system=systemName, category=category, message=message, exception=e))
            return False
        return True
...
def exception(system, message, exception):
    ...
    if _instance is not None:
        ...
        print("Log not initialized: {system}: {message}".format(
