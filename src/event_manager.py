...
is found within obj's implementation."""
    ...
    if obj is not None:
        logger.debug("event_manager", "addListener({name})".format(
...
"""
    ...
    if type not in [SCENE_INTERVAL_TICK]:
        logger.debug("event_manager", "Posted event {evt}".format(
            evt=EVENT_NAMES.get(type, "UNKNOWN")))
...
def dispatch(event):
    ...
    will_script = "event_will_%s" % EVENT_NAMES.get(
        event.get("type", 'unknown'), None)
    did_script = "event_did_%s" % EVENT_NAMES.get(
        event.get("type", 'unknown'), None)
    ...
    for list in [will_chain, do_chain, did_chain]:
        for method, listener in list:
            ret = logger.debug("event_manager", "{obj}.{script}".format(
                obj=listener.__class__.__name__, script=method.__name__)) if not "interval" in method.__name__ else None
            try:
                ret = method(event.get('data', None))
            except Exception as e:
                logger.exception("event_manager", "Failed to execute {name}.{script}({event}): {exception}".format(
                    name=listener.__class__.__name__, script=method.__name__, event=event, exception=e), e)
                ret = False
            if ret is False:
                logger.debug("event_manager", "{obj}.{script}: blocked event propagation".format(
                    obj=listener.__class__.__name__, script=method.__name__))
                return
