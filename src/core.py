...
class AGE():
    ...
    def run(self):
        ...
        logger.initialize()
        if gameconfig.initialize("tss.json") is False:
            logger.error("main", "Invalid configuration.")
            return
        if speech.initialize() is False:
            logger.error(
                "main", "Failed to start due to a lack of speech support.")
            return
        speech.cancelSpeech()
        ...
        if scene_manager.load_scene(gameconfig.get_start_scene()) is False:
            print("Failed to load first scene {name}".format(
                name=gameconfig.get_start_scene()))
            return
        self.stop_animation()
        ...
        while 1:
            ...
            if now - old_time > constants.INTERVAL_TICK_RESOLUTION:
                ...
                event_manager.post(event_manager.SCENE_INTERVAL_TICK, {
                                   "time": self.current_ticks})
                old_time = now
            time.sleep(0.01)
