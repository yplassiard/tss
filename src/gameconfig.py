...
class GameConfig():
    ...
    def load_scene_configuration(self, json_file):
        ...
        try:
            ...
            logger.error(self, "Failed to load {file}: {exception}".format(
                file=json_file, exception=ex))
            return None
...
def get_value(config, key, cls, attrs=None):
    ...
    if isinstance(ret, cls) is False:
        raise RuntimeError("Configuration error: {key} has to be {clsName}".format(
            key=key, clsName=class_name))
    if attrs is None:
        return ret
    ...
    if min_value is not None and min_value > ret:
        raise RuntimeError(
            f"Configuration error: The minimum allowed value for {key} is {min_value}")
    if max_value is not None and max_value < ret:
        raise RuntimeError(
            f"Configuration error: The maximum allowed value for {key} is {max_value}")
